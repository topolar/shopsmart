...
Identitu zajišťuje Google-only Firebase Authentication. Fastify přes Firebase Admin ověří čerstvý ID token, vydá host-only HttpOnly session cookie a z ověřeného Firebase UID dohledá lokálního uživatele a tenant v PostgreSQL. Firebase neukládá ShopSmart preference ani autorizační stav. Bezpečnostní kontrakt a lokální provoz popisují [ADR 0007](decisions/0007-firebase-google-authentication.md) a [Firebase Auth runbook](FIREBASE_AUTH.md); původní [ADR 0002](decisions/0002-authentication-and-sessions.md) zůstává jen jako superseded historie.
